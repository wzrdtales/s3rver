...
class S3rver extends Koa {
  constructor(options) {
    ...
    this.store = this.context.store = externalStore
      ? new externalStore(directory)
      : new FilesystemStore(directory);
...
function onerror(err) {
  ...
  if (err.code == 429 && !(err instanceof Error)) {
    err = new S3Error('TooManyRequests', err.error_summary, err);
  }
